resource "aws_instance" "joey_instance" {

  count = length(var.ec2_instance_name)
  # This ami is used to launch an EC2 instance with Amazon Linux 2
  # The value came from the data.tf
  ami                         = data.aws_ami.amazon_linux_2.id
  instance_type               = "t2.micro"
  associate_public_ip_address = true
  vpc_security_group_ids      = [var.my_sec_group_id]
  subnet_id                   = var.my_subnets[count.index]
